...
following command: msfvenom --list payloads | grep linux/x86
...
We choose to execute /bin/sh as binary.

msfvenom -p linux/x86/exec -a x86 --platform linux CMD=/bin/sh -f c -o exec_sc_with_null.txt

I started GDB and arrived to pause the execution when the payload it has been
executed.
...
Then we convert the doubleword value in EAX into a quadword using the CDQ
instruction.
...
![exec_with_null_register_before_execve.png]
